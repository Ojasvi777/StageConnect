...
import { createEducation, updateEducation } from "../Actions/profile";
...
export default function EducationModal({ isOpen, onClose, userId, education }: EducationModalProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    startDate: education?.start_date ? new Date(education.start_date).toISOString().split('T')[0] : "",
    endDate: education?.end_date ? new Date(education.end_date).toISOString().split('T')[0] : "",
    description: education?.description || "",
    isCurrent: !education?.end_date,
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (education) {
        // Update existing
        result = await updateEducation(education.education_id, data);
      } else {
        // Create new
        result = await createEducation(userId, data);
      }
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="School, academy, workshop"
            />
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={handleChange}
                className="w-full px-4 py-2 rounded-lg border border-[#F3E6C9] focus:outline-none focus:ring-2 focus:ring-[#D4AF37]"
                placeholder="e.g., Classical Dance, Vocal Music"
              />
            </div>
          </div>
